...
- `src/backend/services/llm/ollama.js`: An OpenAI-compatible Chat Completions client for Node.js that reads environment variables:
...
- `src/backend/services/llm/ollama_cli.mjs`: A small Node CLI that imports the helper above and performs a single chat completion from a provided sequence of strings. The backend uses this via `subprocess.run()`.
